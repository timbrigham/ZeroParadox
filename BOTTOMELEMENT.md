...
reading that the various bottoms are *one object* is retired as ill-typed (object equality across categories does not typecheck and is not invariant under equivalence); what separates the members is proved property by property (`seam_unique_among_named`, for the named bottoms, in a lattice with no top). It closes a standing gap: a framework built on ⊥ that had not yet characterized ⊥ itself.
...
- `GEN` ∅ - self-coincident fixed point (reading ⊥ as the quine itself is the [`KleeneStructure`](ZeroParadox/Computability/Kleene.lean) requirement, [`botCode_is_quine`](ZeroParadox/Computability/Kleene.lean), not a theorem) - carries SELF, not floor-generates-tower
