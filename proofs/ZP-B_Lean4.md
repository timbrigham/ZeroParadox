...
|-----------|-----------|-----------|
| AX-B1 (binary existence) | `ax_b1_distinct` | `nullState ≠ firstAtomicState` — proved by `decide` via decidable equality on Fin 2; depends only on `propext`, not `Classical.em`. Not a novel axiom of this framework. |
