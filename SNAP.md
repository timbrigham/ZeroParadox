...
**The family (proved), the identity (retired), minimality (open).** The domain snaps form one **family** (MC-1): per-domain membership is proved, the reading that they are *numerically one* transition is retired as ill-typed (a type boundary, not a theorem), and the members are provably distinct. Separately, whether ε₀-as-least-fixed-point is choice-free at the notation level is open: the syntax-to-semantics bridge [`tower_NF`](ZeroParadox/Ordinal/ConstructiveOrdinals.lean) inherits `Classical.choice`.
