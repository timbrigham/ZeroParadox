...
Machine-checked proofs of the formal documents using Lean 4 + Mathlib. Source lives under `ZeroParadox/` in this repository. The full theorem-by-theorem detail is in each source file.
...
| OQ-G3: Functor construction | **Closed (functor construction) - ZP-H functor files, June 2026.** F_A: NatSLat appendix (ℕ with max/0 as ZPCategory; 0 categorical initial). F_B/C/D now have sorry-free Lean `CategoryTheory.Functor` terms into the standard Mathlib categories, each carrying the snap floor to that category's categorical bottom: F_B `fB_functor : ℕᵒᵖ ⥤ TopCat` (clopen balls of Q₂; ⊥ is the inverse limit ⋂ B(0,2⁻ⁿ) = {0}), F_D `fD_functor : ℕ ⥤ ModuleCat ℂ` (state spaces; ⊥ = StateSpace 0 is the initial zero object; embeddings proved isometric), F_C `fC_functor : ℕ ⥤ KleisliCat PMF` (stochastic maps; ⊥ = Fin 0 is initial, with no morphism returning to it - AX-G2 as a theorem). Mathlib has no bespoke pTop/Hilb/InfoSp categories, so these standard categories (topological spaces, ℂ-modules, the Kleisli category of the probability monad) are the genuine realizations; they are not ZPCategory instances (each has a terminal object), so the snap floor is the limit in TopCat and the initial object in the other two. The earlier ℕ-shaped proxy witnesses (Q₂BallDepth, InfoDepth, HilbDimDepth) are superseded. The cross-category *identity* of these bottoms (MC-1's identity half) remains a chosen identification. |
...
| Formal verification (Lean/Rocq) | ZP-A through ZP-M verified; ZP-M adds snapEmbed type bridge (MachinePhase → ℤ_[2]), hfp closure, zpm_triangle (ordinal-2adic-phase triangle), both_fixed_points_exist (Kleene-ordinal structural homology) - May 2026; ZP-H adds the native-category functors (fB_functor / fD_functor / fC_functor into TopCat / ModuleCat ℂ / KleisliCat PMF) and the mc1_correspondence capstone - June 2026 |
| Classical.choice necessity | **Open** - ZP-L, ZP-M, and the ZP-H native-category functors inherit Classical.choice via Mathlib ordinal, analysis, and topology/PMF infrastructure. Whether the dependency is structurally forced by ZP snap geometry or incidental to Mathlib's implementation is an open question. Testable via constructive ordinal fixed-point theory over ONote/NONote (Lean 4 CNF notation). Future ZP-N (constructive validation layer). |
...
| DA-1: Instantiation alignment | **Closed given DP-2 - ZP-E / ZP-K.** da1_minimal_path proved axiom-free in Lean; Paths 1 and 3 closed via da1_closed_concrete (ZP-K). The former Path 2 (the AIT bridge between ZP-C and computability) is not an open problem but a foundational commitment, carried by DP-2's motivational grounding - a chosen principle, not a proof gap (the same kind of standing residual as MC-1's identity half). |
