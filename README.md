...
|------|----------|---------|----------|
| [ZP-A Lattice Algebra](ZP-A_Lattice_Algebra_v1_8.pdf) | ZP-A | v1.8 | Join-semilattice (L, ∨, ⊥). Axioms A1-A4. Monotonicity. Additive ontology. CC-2: ⊥ = {⊥} (Quine atom, ZF + AFA). R3: CC-2 eliminates static-description state for ⊥, given D7 exhaustiveness (ZP-E) as background. v1.8: CC-2 cross-framework note added — Foundation ruled out by R3 and L-INF; AFA identified as forced; cross-reference to ZP-E Remark R-AFA. |
...
| [ZP-E Bridge Document](ZP-E_Bridge_Document_v3_7.pdf) | ZP-E | v3.7 | **DA-1** (instantiation = execution; DP-2 (Execution Distinguishability) — TrackedOutput separates output value from machine state; da1_minimal_path proved axiom-free in Lean. Three informal paths explicitly framed as corroboration of DP-2's precondition, not parallel proofs). **T-SNAP** (Binary Snap derived). **DA-2** (instantiation succession, directed tree). **DA-3** (perspective-relative cardinality). Remark R-ε₀ (ε₀ symbol justified). Remark R-AFA (Foundation ruled out; AFA forced). v3.7: DA-1 formally closed via ZP-K — da1_closed_concrete : IsQuineAtom(⊥ : MachinePhase) proved in Lean 4; Paths 1 and 3 IN LEAN SCOPE; "Outside Lean Scope" designation removed. v3.6: DA-1 Path 1 rewritten — self-execution argument leads; ZP-J T-EXEC cited as formal verification; CC-1 and CC-2 no longer freestanding commitments. Full traceability register. |
...
| [ZP-I Inside Zero](ZP-I_Inside_Zero_v1_2.pdf) | ZP-I | v1.2 | **T-IZ (Inside Zero):** every maximal ascending chain is a Cauchy sequence converging to its own successor null in Q2. ZP-A R1 (no top) is the engine: unbounded ascent forces v2(Sn) → infinity, which is the Cauchy condition. Topological core (t_iz_cauchy, t_iz_valuation_unbounded) proved axiom-free in Lean; full conclusion (generation of ⊥') depends on valuation-complexity bridge (outside Lean scope, same status as DA-1 Path 3). OQ-E2 partially closed — ordinal indexing Omega = omega forced by countable binary substrate. v1.2: t_iz_valuation_unbounded proves "sup v2 = ∞" from h_strict; proof obligation table row 3 closed. |
...
| [ZP-K Computational Grounding](ZP-K_Computational_Grounding_v1_1.pdf) | ZP-K | v1.1 | **T-COMP (Computational Grounding):** (1)-(3) equivalent by T-EXEC (Quine atom = ⊥ = join identity); (4) Kleene fixed point combined by KleeneStructure typeclass requirement. KleeneStructure typeclass bridges AFAStructure to computability. MachinePhase instances: machinePhaseAFA + machinePhaseKleene. **DA-1 closed concretely:** da1_closed_concrete : IsQuineAtom(⊥ : MachinePhase). selfApply_partrec proved via Kleene's second recursion theorem. v1.1: Remark R-K.0 added — four-way equivalence clarified: (4) is a typeclass structural commitment, not independently derived from (1)-(3). |
...
| DA-1: Instantiation alignment | Closed - ZP-E DA-1 v3.7 / ZP-K (Derived Proposition: DP-2 formal core — da1_minimal_path proved axiom-free in Lean; Paths 1 and 3 formally closed via ZP-K — da1_closed_concrete : IsQuineAtom(⊥ : MachinePhase); KleeneStructure MachinePhase instance; Path 2 (informational bridge) outside Lean scope) |
| CC-1 (S₀ = ⊥) derivability | **Closed - ZP-J cc1_derived (axiom-free, Lean)** — was ZP-A Conditional Claim; now a derived theorem |
...
| OQ-E2: Cardinality-semilattice correspondence | **Partially closed - ZP-I T-IZ v1.2** - ordinal indexing Omega = omega forced by countable binary substrate (ZP-C D4, Q2 separability, binary alphabet). Internal/external perspective relativity is ordinal, not set-theoretically free. Formal connection between specific semilattice structures and specific CH instances remains open. |
...
| T-IZ: Inside Zero Theorem | **Derived - ZP-I v1.2** - every maximal ascending chain converges to its own successor null. Topological core (t_iz_cauchy, t_iz_valuation_unbounded) proved axiom-free in Lean; full conclusion (generation of ⊥') depends on valuation-complexity bridge (outside Lean scope, same status as DA-1 Path 3). |
...
**Mathematician:** [ZP-A](ZP-A_Lattice_Algebra_v1_8.pdf) → [ZP-B](ZP-B_pAdic_Topology_v1_4.pdf) → [ZP-C](ZP-C_Information_Theory_v1_9.pdf) → [ZP-D](ZP-D_State_Layer_v1_6.pdf) → [ZP-E](ZP-E_Bridge_Document_v3_7.pdf) → [ZP-J](ZP-J_Self_Reference_v1_1.pdf) → [ZP-K](ZP-K_Computational_Grounding_v1_1.pdf) → [ZP-I](ZP-I_Inside_Zero_v1_2.pdf) (framework closure)

**For the category theory extension:** [ZP-G](ZP-G_Category_Theory_v1_4.pdf) → [ZP-H](ZP-H_Categorical_Bridge_v1_7.pdf) (self-contained; after ZP-E)
