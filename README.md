...
| [ZP-J Wheel Addendum](ZP-J_Wheel_Addendum.pdf) | ZP-J Wheel Addendum | v1.0 | **The wheel of fractions is a wheel.** For any commutative ring A and multiplicative submonoid S, the wheel of fractions ⊙_S A = (A×A)/≡_S satisfies all of Carlström's Definition 1.1 wheel axioms (eight axioms, 14 unbundled fields) - WheelFrac.instWheel. inf_ne_bot: the reciprocal of zero (∞ = /0) and the absorbing element (⊥ = 0·/0) stay distinct given 0 ∉ S, so the construction is a wheel, not a meadow. Choice-free: [propext, Quot.sound]. The algebraic form of the ZP porthole; a machine-verified encoding of Carlström's construction. Lean-verified sorry-free (ZPJ_WheelFrac). Reads after ZP-J Self-Reference. |
...
| ZP-J Wheel of Fractions | [ZPJ_Wheel.lean](ZeroParadox/ZPJ_Wheel.lean), [ZPJ_WheelFrac.lean](ZeroParadox/ZPJ_WheelFrac.lean) | **Wheel** typeclass (faithful encoding of Carlström 2001:11 Definition 1.1 - all eight axioms, his two commutative-monoid axioms unbundled into 14 equational fields), ZPWheelElem concrete instance, porthole theorems (zpw_inv_zero_eq_inf, zpw_zero_mul_inf_eq_bot, zpw_top_val_iff_inv_is_inf), WheelValuationStructure bridge typeclass; **WheelFrac.instWheel** (the wheel of fractions ⊙_S A = (A × A)/≡_S is a Wheel for any commutative ring A and multiplicative submonoid S), **WheelFrac.inf_ne_bot** (∞ ≠ ⊥ given 0 ∉ S - the wheel-vs-meadow discriminator); Classical.choice-free ([propext, Quot.sound]) | Clean - June 2026 |
