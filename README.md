...
The bottoms across the layers form one characterized **family** (MC-1): per-domain membership is proved (the categorical criterion is [`mc1_correspondence`](ZeroParadox/Multihomed/MC1Bridge.lean)), while the reading that they are numerically *one object* is retired as ill-typed (object equality across categories does not typecheck and is not invariant under equivalence); what separates the members is proved property by property ([`seam_unique_among_named`](ZeroParadox/Category/SeamUniqueness.lean), for the named bottoms, in a lattice with no top). ε₀ is chosen as the proof-theoretic ordinal of PA - a cited classical fact the framework invokes, not one it re-proves, and it takes more than one of Gentzen's results: transfinite induction up to ε₀ suffices to prove the consistency of PA (Gentzen, Math. Annalen 112 (1936) 493-565); that induction is not itself provable in PA, which follows indirectly from Gödel's theorem together with the 1936 result and which Gentzen proved directly in Math. Annalen 119 (1943) 140-161; and transfinite induction up to any ordinal strictly below ε₀ is provable in PA, which Gentzen reports as already known, crediting Hilbert-Bernays. Sufficiency scales upward, so the 1936 bound alone is equally true of any larger ordinal. What pins the value to exactly ε₀ is the other two together, and they run in opposite directions: PA proves transfinite induction at every ordinal strictly below ε₀, and does not prove it at ε₀ itself. Its role as the exact snap threshold is machine-verified, and the one open residue is the type-level identity across universes (OQ-E2), not the Gentzen relationship itself. The full labelled account is in [Axiomatic Commitments](#axiomatic-commitments).
...
→ **T-SNAP** (Binary Snap derived: A4, the standard bottom element axiom, gives its join, AX-B1 supplies the atom, and `t_snap_given` takes the start at ⊥ and occurrence at the first step as hypotheses; AX-1 is retired: the snap's shape is this theorem, and that the snap occurs is stated separately: it follows from the occurrence commitment, that instantiation occurs, together with DA-1, closed given DP-2)
...
|------|----------|---------|-------|
| [Lattice Algebra](ZP-A_Lattice_Algebra.pdf) | ZP-A | v1.29 | The lattice-algebra foundation: the bottom element ⊥ and the order it induces. |
...
| [Native Categories Addendum](ZP-H_Native_Categories_Addendum.pdf) | ZP-H Native Categories Addendum | v1.6 | The snap floor realized inside each framework's native Mathlib category (TopCat, ModuleCat ℂ, KleisliCat PMF). Reads after ZP-H. |
...
| [Incomputability Convergence](ZP-L_Incomputability_Convergence.pdf) | ZP-L | v1.18 | ε₀ as the exact ordinal threshold the snap is keyed to. (That the snap occurs follows from the occurrence commitment together with DA-1, closed given DP-2.) |
...
| [The Frame-Change](ZP-Q_The_Frame_Change.pdf) | ZP-Q | v1.11 | Synthesis layer (ZP-P sequel): the bottom read as both 0 and ∞, and what that does not settle. The order-theoretic frame-flip universal is proved; the categorical Lawvere universal meets a proven Cantor wall; the cross-domain identity is a type boundary. |
...
This framework adds no axioms specific to the result: the central theorem takes its join from the standard bottom-element axiom of join-semilattice theory and its atom from AX-B1. Its start at ⊥ and occurrence at the first step are stated as hypotheses in `t_snap_given`, which binds no atom. **AX-1 (Binary Snap Causality) is retired.** Its content was split in two: the shape of the snap is proved, as Theorem T-SNAP, and that the snap occurs is stated separately: it follows from the occurrence commitment (instantiation occurs) together with DA-1 (closed given DP-2). `tsnap_holds_but_nothing_moves` shows T-SNAP does not carry it. T-SNAP is derived in ZP-E.

Of the remaining commitments, most are restatements of structure established in earlier layers (AX-G1 and AX-G2 are grounded in ZP-A's bottom element and ZP-A antisymmetry with ZP-B C3). **One is substantive and is not reducible to computation: AX-B1**, that existence is discrete rather than a continuum of partial states. AX-B1 holds at every state with anything above it: each has a first distinct state above it, with nothing strictly between. A state with nothing above it owes no step. The `decide` proof (`ax_b1_distinct`) checks that the two states are distinct *given* a two-element type; choosing a discrete alphabet over a continuum is the commitment itself, and is not what `decide` verifies. See the [Claims Ledger](CLAIMS.md) for the full statement.
