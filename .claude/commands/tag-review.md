...
## Output
Write a decision-log table (§10 schema: `decl · SSOT · deepseek · opus · verdict · lane · proposed-tag · confidence · date · source_sha256`) to `.claude-local/notes/tag_review_log.md`. Present the CORRECT/SPLIT list to Tim. **Stop. Apply nothing until he signs off.** On approval: `annotate` the approved CORRECTs → `ssot_l1_acceptance.py` exit 0 → re-pin the new hash → finalize the log → export the SSOT with `export_full` to the **ABSOLUTE** path `C:/Workspace/ZeroParadox/ssot.json` (the canonical store) **if any retag was applied**, and commit it with the change.

⚠ **Two ways that step used to be a no-op.** It said *"if declarations changed"* — but a tag review changes **annotations**, never declarations, so the guard was false by construction and the canonical store was never written; the approved retags stayed inside the MCP. And it named `ssot.json` **relative**, which `export_full` resolves **beside the MCP** rather than in the repo, so even a run that got past the guard wrote to the wrong file. Both are the same shape as the dead `tools/registry/registry_export.json` path this step replaced: a final write that reports success into a location nothing reads.
