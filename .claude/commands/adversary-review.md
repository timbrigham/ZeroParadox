...
**FIRST — CALLER ONLY: run `python tools/verify/gate_round.py bump` ONCE per round and put the number in
the brief. The reviewer must NEVER bump** — a spawned agent that bumps double-counts the round and burns
...
- the **formal document build scripts** (`.claude-local/build_zp*.py`, mirrored in `scripts/`) — every `body()`, `label_box()`, `sp()` and box-helper string that renders as PDF prose. Prioritize sections named "Open Questions", "Boundary Conditions", "Scope", "Notes", and any "Status:" / "Note on ..." string: **that is where a derivation gets asserted in passing rather than stated as a labelled result, so it carries no Theorem/CC/DP label for anything else to check.**
