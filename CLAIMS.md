...
| **Carboni-Lack-Walters (1993)** strict initial objects; **Fritz (2020)** Markov categories (Kleisli of the Giry monad) | ZP-G / ZP-H: ⊥ as the (strict) initial / categorical bottom across the domain categories; AX-G2 = strict initiality; `mc1_correspondence` | **Theorem-grounded + commitment** - each domain bottom is the categorical bottom of its own real Mathlib category (per-category Lean: `fD_zero_isInitial`, `fC_zero_isInitial`, the TopCat limit); the cross-category "one object" identity is the MC-1 commitment | Built on the cited work - standard categorical structure, assembled across the ZP domains |
