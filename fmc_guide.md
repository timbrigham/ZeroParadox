...
A related move, and unlike the fork above it is not a choice at all: this same shape turns up in several different areas of mathematics, wearing a different costume in each: a self-copying program, a self-containing collection, a point where a certain scale runs to infinity. The framework gives that shared shape a single name and uses it throughout. A name is not a claim. It is not proved, forced, chosen or open, because there is nothing in it to be right or wrong about, and the only fair question to ask of it is whether it earns its keep. Mathematics does this constantly: "normal" means one thing of a subgroup, another of a topological space, and another of an operator, and nobody reads the shared word as a claim that the three are one concept. Whether the costumes are literally the same object is a different question, one that does not even parse across areas this different, so the framework does not ask it and does not need to. Seeing arguments as different as Cantor's, Russell's, Gödel's and Tarski's as one shape is not this framework's idea. Lawvere gave it a general form, and a whole family of people has worked on it since. This is a line of work the framework joins rather than starts.

None of the framings in this section, "does nothing contain nothing" or "a bottom that contains itself," are proofs. They are ways to understand the choices the framework makes; the naming described just above is not even a choice, only a name. The proofs live elsewhere, and they are labeled proofs.
