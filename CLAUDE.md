...
**The trinary of the keystone — μ / ν-hosted / ν-refused. Read this before writing about the "one root
or two" question, about well-foundedness as a "second root," or about how many faces self-reference has.**
Authoritative Lean: `ZeroParadox/Settheory/HostVerdict.lean` (built green and sorry-free 2026-07-29; the
hinge and both ν faces **axiom-free**, the two `ZFSet`-touching results `[propext, Quot.sound]`,
choice-free).

The one engine (`negation_no_fixedpoint` → `lawvere_fixedpoint`) forks into **two regimes**, stated at
`Settheory/Wall.lean:123-124`: **μ** — the map is fixed-point-free, so no object forms; **ν** — a fixed
point exists (the self-referential object). Counting the ν object as a face of its own gives **THREE**,
and **two of the three ARE the self-reference object** — the same object seen hosted and seen refused:

| Face | What happens to the fixed point | Witness |
|---|---|---|
| **μ — the wall** | no object ever forms | `cantor_via_engine`, `russell_via_engine`, `no_self_decider` |
| **ν-hosted — it lives** | host carries the self-loop, so host is NOT well-founded | `nu_hosted_face` (= `floor_not_wellFounded`, from ⊥'s `fixed_bot`) |
| **ν-refused — it is forbidden** | host IS well-founded, so no self-loop | `nu_refused_face` (= `no_quine_atom` = `wf_no_selfloop ZFSet.mem_wf x`) |

**The load-bearing fact: the two ν faces are ONE theorem read in its two directions.** The hinge is
`host_verdict_axis : ¬ (WellFounded r ∧ r x x)` — `wf_no_selfloop` as a mutual exclusion. So
**well-foundedness is NOT a third root of self-reference; it is the AXIS on which a host renders its
verdict on ν.** "Root B" was never a root — every theorem assigned to it is that one hinge instantiated
at some host relation.

**Four fences, and violating any of them re-collapses the carving:**
- **The narrow one-root question is still answered NO.** `wf_no_selfloop` is proved by **accessibility**,
  not by the engine, and nothing derives it from the engine. Two *mechanisms* remain;
  `selfloop_permitted` / `engine_is_wf_free` (`Wall_OneRoot.lean`) stand. The claim is only that one of
  the two mechanisms is a *verdict on the engine's output*, not a rival root.
- **`trinary_faces` is a CONJUNCTION, never an identity.** Three carriers (`A → (A → Prop)`, `L`,
  `ZFSet`); a cross-carrier `=` is ill-formed. "Both hold" ≠ "the same fact" (the `da1_paths_unified`
  lesson).
- **The shadow/literal distinction is PRESERVED, not crossed** (`Wall.lean:17-21`). `floorRel` (the
  `selfApp` graph) and `ZFSet` membership are different relations; the hinge reaches both only because it
  quantifies over *any* relation. It does not identify them.
- **No traversal.** Nothing here says the object moves, or that ν-hosted implies a first step is taken.
  That is the standing commitment (`l_inf`'s docstring). ν-hosted is the only face where the object could
  move — which is why `Wall.lean:90` calls the computability row "the pivot" — and even there
  distinguishability is proved (`succession_lt_succ`) while forced traversal is not.

**Why this is here (2026-07-29).** The reframe is Tim's, and it was reached by asking whether the binary
framing was hiding a trinary "where one of the three is specifically the self-reference object" — the same
instinct that resolved `Occurrence.lean`'s trichotomy. It was tested against the Lean and the factoring
succeeded; the prediction that the shadow/literal gap would block it was **wrong**. Note: `HostVerdict.lean`
adds no new mathematics — every component was already proved. The contribution is the **carving**, which is
exactly the kind of thing that gets re-collapsed to "one root or two" by a future session reading
`Wall.lean` alone. Full development: `.claude-local/notes/wall_one_root_or_two_trinary_2026-07-29.md`.
...
- **Step 0 — grep our own corpus first.** Before any web search, `/prior-art-review` greps the repo + `.claude-local` (notes, **`papers/`**, `external/`, outreach) for an existing reference; much of this project's prior-art knowledge already lives there, so this prevents false-positive "gaps" — e.g. the Bruhat-Tits tree is already cited in `PadicTree.lean`, and a web-first sweep "rediscovered" it. **`.claude-local/papers/` is the downloaded-source library (55 PDFs as of 2026-07-26) and is the FIRST place to look for a book or paper — check it before concluding a source cannot be obtained.** Measured 2026-07-26: a scout spent a full search declaring Aczel's *Non-Well-Founded Sets* unobtainable (404s, dead mirrors, lending-restricted archive.org) while `.claude-local/papers/aczel_afa_manuscript.pdf` sat on disk — because this line listed `external/` and omitted `papers/`, and the brief inherited the omission. **Carry `papers/` into every scout brief explicitly.** Note also that scanned books here are OCR'd with spurious intra-word spaces ("depend ent choice s"), so **grep loosely** — a miss on a tight pattern is not evidence of absence.
