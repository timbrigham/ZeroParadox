...
- **The equivocation (hit four separate times on 2026-07-19).** The ordinary English "choice" — an act of picking, a point of view, a chart selection — is **not** the axiom `Classical.choice`. Choice is strictly stronger than excluded middle (Diaconescu 1975: choice ⇒ LEM, one direction; converse fails). Every evocative "choice = which way you view the split" reading is a **model** of the choice-vs-no-choice distinction, never the axiom. State it as a model or not at all.
- **The census — measure, never quote.** A live full build reports **882** `#print axioms` results, **658** carrying `Classical.choice`, **135** fully axiom-free, across ~150 files. A note previously claimed "22 of ~1266 (~1.7%)" — **wrong by ~29x**, and wrong precisely because it was quoted rather than measured. The index carries the PowerShell to regenerate the count; re-run it rather than citing any number, including these. **The framework is not choice-free — it is majority-choice-carrying by proof footprint**, and the accidental/essential classification covers only ~3% of it.
