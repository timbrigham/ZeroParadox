...
- `ZeroParadox/Category/ChoiceCannotBe.lean` - Machine-checked characterization index of the framework's **relationship to `Classical.choice`** - the fourth `CannotBe` index, after ⊥, the snap, and ε₀. **A framing difference from the other three:** choice is NOT a framework object, it is an ambient kernel axiom, so this indexes where choice is provably not needed, what it must not be confused with, and what is actually established. **Headline fence:** the English word "choice" (an act of picking, a point of view, a chart selection) and the axiom `Classical.choice` are NOT the same thing; choice is strictly stronger than excluded middle (Diaconescu 1975, one direction), and every "choice = which way you view the split" reading is a MODEL of the choice-vs-no-choice distinction, never the axiom. `#check`-only - creates no declarations, so it structurally cannot overclaim, and the `import`s force every indexed proof to recompile. Carries the honest census (**882 reports, 658 carrying choice, 135 axiom-free**, ~150 files - correcting a prior note's "22 of ~1266, ~1.7%", wrong by ~29x) **with the PowerShell to reproduce it**, since that figure was wrong precisely because it was quoted rather than measured. Indexes both directions: the choice-free results (§ I) and an actual choice-carrying case (`cofix_nonempty'`) beside its axiom-free counterpart, so the file cannot read as "the framework is choice-free" - it is not.
...
- `ZeroParadox/Ordinal/SnapSuccession.lean` - The succession as a strict chain: the new bottoms are the ε-numbers (`ε_ = deriv (ω^·)`), which are exactly the closed points of `snapNucleus` (`snapNucleus_isClosed_iff`), climbing strictly (`succession_lt_succ`), each the snap re-seeded one step above the last (`succession_succ`). The orthogonality of the rungs is realized in the Hilbert chart (ZP-D `t5_strict_orthogonal`); the cross-chart bridge is `Multihomed/SeparatedSuccession.lean`.
