...
- `ZeroParadox/Ordinal/SnapMetaLattice.lean` - The lattice of systems: adjoining the point at infinity (`WithTop Ordinal`) makes the ordinals a frame, on which the nuclei form a `Frame` and the sublocales a `Coframe` — the lattice of systems the bare (top-less) ordinals lack. Provably the top's doing: the machinery fires on `WithTop Ordinal`, not on bare `Ordinal` where `snapNucleus` still lives. `example`-only; instantiates the meta-level.
