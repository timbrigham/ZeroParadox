...
- `ZeroParadox/Category/DoubleNegationNucleus.lean` - Choice as a difference-generator: `dnegNucleus : Nucleus X` (the double-negation map `a ↦ aᶜᶜ` on any Heyting algebra), the choice-side parallel of `snapNucleus`. Its closed points are exactly the regular elements — the Boolean core (`dnegNucleus_isClosed_iff`). Textbook (¬¬ subtopos / Glivenko); the contribution is the object, named as the framework's choice modality beside the snap.
