...
/- Lean Scope Note — T6-b/c versus PDF Claims
   ...
   This is the same category as DA-1 Path 3 (the AIT bridge) in the formal document **ZP-E §VI** —
   a PDF cross-reference, not a Lean file. (Corrected 2026-08-01: this read `ZPE.lean §VI`, naming a
   Lean file for content the very next clause says exists only in the PDF. The pre-reorg `ZPE.lean`
   *did* exist — the SJV records it, and its declarations now live in `ZeroParadox/Order/Snap.lean` —
   but the AIT material was never in it. The cited prose is generated by `scripts/build_zpe.py:272`,
   "Path 3 — Formal bridge: Incompressibility as Self-Description (ZP-C D1 + standard AIT): ".) The
