...
`node4_generates_nat` proves `ℕ` is the colimit of the successor chain rooted at the floor `Fin 0`,
choice-free — ONE concrete Adámek instance (`X ↦ X + 1`), not the general theorem. Scope, the Mathlib
comparison and the dated library survey: `ZeroParadox/Category/Node4Generation.md`.
