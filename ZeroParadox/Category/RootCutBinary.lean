...
-- where the axiom originates) and `strict_cofix_nonempty` proves the same inhabitation with no axioms
...
-- and his own preferred coinductive construction needs neither choice nor LLPO).
