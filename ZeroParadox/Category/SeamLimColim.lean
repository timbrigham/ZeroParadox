...
TC12/TC13 (`hilbert_bottom_isZero`) established the **object-level** seam: node #5,
...
1. **`seam_empty_lim_eq_colim` — the genuinely seam-specific case (the real content).** For the *empty*
   ...
   `Z_SeamLimColim = fD_functor.obj 0` these are the SAME object `Z_SeamLimColim`: `IsLimit (asEmptyCone Z_SeamLimColim)` (≡ `IsTerminal Z_SeamLimColim`)
   AND `IsColimit (asEmptyCocone Z_SeamLimColim)` (≡ `IsInitial Z_SeamLimColim`) both hold, both cones with apex `Z_SeamLimColim`. So the
   empty-diagram limit and colimit coincide on `Z_SeamLimColim`. This is the diagram-level seam coincidence, and it
...
3. **`const_connected_lim_eq_colim_generic` — the fence on the OTHER overclaim (honesty about scope).**
   ...
   non-discrete J the colimit escapes Z_SeamLimColim via a copower") is FALSE for connected J: the colimit of a
...
/-- The Hilbert bottom, the seam node `Z_SeamLimColim = fD_functor.obj 0`. -/
noncomputable abbrev Z_SeamLimColim : ModuleCat ℂ := fD_functor.obj 0

/-- **(1) Seam diagram-level coincidence.** At the zero object `Z_SeamLimColim`, the empty-diagram **limit** and the
    empty-diagram **colimit** both land on `Z_SeamLimColim`: `asEmptyCone Z_SeamLimColim` is a limit (≡ `Z_SeamLimColim` terminal) and
    `asEmptyCocone Z_SeamLimColim` is a colimit (≡ `Z_SeamLimColim` initial), and both cones have apex `Z_SeamLimColim`. So at the seam the
    limit and colimit of the empty diagram coincide as the single object `Z_SeamLimColim` — the μ=ν object-level
    coincidence lifted to the diagram level. -/
theorem seam_empty_lim_eq_colim :
    ∃ (_ : IsLimit (asEmptyCone Z_SeamLimColim)) (_ : IsColimit (asEmptyCocone Z_SeamLimColim)),
      (asEmptyCone Z_SeamLimColim).pt = (asEmptyCocone Z_SeamLimColim).pt := by
  have hZ : IsZero Z_SeamLimColim := hilbert_bottom_isZero
