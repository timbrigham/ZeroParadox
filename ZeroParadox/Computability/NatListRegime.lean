...
* **pure-strict** (`idPF_NatListRegime`, identity functor `X ↦ X`): `Fix` is empty, `Cofix` is inhabited — the
  strict μ/ν gap.

This file tests a **third regime**, the nat/list functor `natPF_NatListRegime = ⟨Bool, fun b => cond b PUnit PEmpty⟩`
...
* `natFix_nonempty` — `Nonempty (Fix natPF_NatListRegime.Obj)`, choice-free (`propext, Quot.sound` only) from the
  leaf via `Fix.mk` of the empty-child node. (Distinguishes natPF_NatListRegime from `idPF_NatListRegime`, whose `Fix` is empty.)
* `natCofix_nonempty` — `Nonempty (Cofix natPF_NatListRegime.Obj)`; an explicit inhabitant `natInfinity`, the
...
* `fixToCofix_not_surjective` — **the load-bearing edge, PROVEN.** The canonical comparison
  `fixToCofix = Cofix.corec Fix.dest : Fix natPF_NatListRegime.Obj → Cofix natPF_NatListRegime.Obj` is not surjective: `natInfinity`
...
**What is Lean vs interpretation.** Lean proves: `natPF_NatListRegime`'s initial algebra is inhabited and its
...
def idPF_NatListRegime : PFunctor.{0, 0} := ⟨PUnit, fun _ => PUnit⟩
...
def natPF_NatListRegime : PFunctor.{0, 0} := ⟨Bool, fun b => cond b PUnit PEmpty⟩

/-! ### Regime 3, base case: `Fix natPF_NatListRegime.Obj` is INHABITED.

This is the structural separation from `idPF_NatListRegime` (pure-strict), whose `Fix` is empty: the `b = false`
leaf node has `PEmpty` children, so `Fix.mk` applies with no recursive arguments. -/

/-- The leaf node of `natPF_NatListRegime` over any carrier: head `false`, empty child map. -/
def leafNode {α : Type} : natPF_NatListRegime.Obj α := ⟨false, fun e => e.elim⟩

/-- **Regime 3 base case.** `Fix natPF_NatListRegime.Obj` is inhabited — the leaf builds a base inhabitant
    choice-free. (Contrast `idPF_NatListRegime`, whose `Fix` is empty: it has no leaf.) -/
theorem natFix_nonempty : Nonempty (Fix natPF_NatListRegime.Obj) :=
  ⟨Fix.mk leafNode⟩

/-! ### Regime 3, infinite element: `Cofix natPF_NatListRegime.Obj` has the all-`true` unfolding.

The coalgebra `fun (_ : Unit) => (⟨true, fun _ => ()⟩ : natPF_NatListRegime.Obj Unit)` keeps choosing the successor
...
def succCoalg : Unit → natPF_NatListRegime.Obj Unit := fun _ => ⟨true, fun _ => ()⟩

/-- `natInfinity` : the all-`true` infinite unfolding, an explicit `Cofix natPF_NatListRegime.Obj` inhabitant. -/
def natInfinity : Cofix natPF_NatListRegime.Obj := Cofix.corec succCoalg ()

/-- **Regime 3 final-coalgebra inhabitant.** `Cofix natPF_NatListRegime.Obj` is inhabited. -/
theorem natCofix_nonempty : Nonempty (Cofix natPF_NatListRegime.Obj) :=
  ⟨natInfinity⟩
...
def fixToCofix : Fix natPF_NatListRegime.Obj → Cofix natPF_NatListRegime.Obj := Cofix.corec Fix.dest
...
inductive EventuallyLeaf : Cofix natPF_NatListRegime.Obj → Prop
  | leaf (x : Cofix natPF_NatListRegime.Obj) (c : cond false PUnit PEmpty → Cofix natPF_NatListRegime.Obj)
      (h : Cofix.dest x = ⟨false, c⟩) : EventuallyLeaf x
  | step (x : Cofix natPF_NatListRegime.Obj) (c : cond true PUnit PEmpty → Cofix natPF_NatListRegime.Obj)
...
theorem fixToCofix_dest_mk (y : natPF_NatListRegime.Obj (Fix natPF_NatListRegime.Obj)) :
...
theorem fixToCofix_eventuallyLeaf : ∀ x : Fix natPF_NatListRegime.Obj, EventuallyLeaf (fixToCofix x) := by
  apply Fix.ind
  intro y hy
...
/-- **Three-regime placement, head-level summary.** The three functors are separated by the head of
    their fixpoint's bottom unfolding: `constPF_unit` and `idPF_NatListRegime` are the pure regimes; `natPF_NatListRegime` is the third,
...
theorem fix_cofix_not_surjective :
    Nonempty (Fix natPF_NatListRegime.Obj) ∧ Nonempty (Cofix natPF_NatListRegime.Obj) ∧ ¬ Function.Surjective fixToCofix :=
