...
computability realization of the POLE EXCHANGE.

`Statement:` INVERSION — the code/data involution exchanges the two roles, and its fixed locus is the
self-application diagonal (`codedata_is_frameflip`).
