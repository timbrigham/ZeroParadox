...
[![Complete Project](https://github.com/timbrigham/ZeroParadox/actions/workflows/lean_action_ci.yml/badge.svg)](https://github.com/timbrigham/ZeroParadox/actions/workflows/lean_action_ci.yml)
...
**Snap.lean / Surprisal.lean — DA-1 Path 2** — L-INF formally proves that surprisal is unbounded at ⊥. The inference from unbounded surprisal to "necessarily executing" is an ontological bridge claim, not a mathematical consequence of L-INF alone. Surprisal.lean's L-INF docstring states this explicitly: "L-INF supplies the formal premise; DA-1 supplies the ontological bridge." Kleene.lean carries DA-1's two witnesses together (`da1_paths_unified`): ⊥ is the AFA Quine atom, and the class names a code satisfying `IsComputationalQuine`. That is a conjunction of witnesses, not a proof that the two paths are one fact - the computational side enters as a `KleeneStructure` requirement, so DA-1's closure rests on that commitment rather than on an independent second proof.
