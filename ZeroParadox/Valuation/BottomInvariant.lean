...
## Formal Overview
A probe: the *weaker invariant* (an invariant measure), not a Lawvere fixed point, may be the universal
recurring across faces. ⚠ EXISTENCE level only — uniqueness (unique ergodicity) is **not** proved, and
Mathlib has no API for it. Instances, the caveat on "universal", and the fences: `ZeroParadox/Valuation/BottomInvariant.md`.
-/
...
/-! ## § XI — The BOTH-AT-ONCE regime: a unit multiplier carries both characters

Multiplication by a 2-adic **unit** fixes the floor (`unitMul_fixes_floor`, so `δ₀` is invariant) **and**
preserves every sphere (`unitMul_norm_const`) — one dynamic carrying both invariant characters, where the
sections above exhibit them on two different maps. ⚠ **The Haar half is NOT formalized here**; only the
`δ₀` half is machine-checked. The three regimes, the closable gap and what sphere-preservation costs:
`ZeroParadox/Valuation/BottomInvariant.md`. -/
...
/-- **The concentrated character, machine-checked.** Multiplication by `u` preserves the Dirac mass at the
    ...
    (`MeasureTheory.Measure.map_dirac`, `Mathlib/MeasureTheory/Measure/Dirac.lean`, `@[simp]`, takes
    `(hf : Measurable f)`) and the
