...
its **minimal** closure. **Do not call it a "ceiling" or "a large ordinal"**: positionally ε₀ is the FIRST — `Ordinal.epsilon 0`, index zero in the
...
**Provenance.** `ZeroParadox/Ordinal/Epsilon0MinMax.lean` § I-b (*"Two fences on the '⊥ the seed'
reading"*, **Tim, 2026-07-31**) already states the implication `a ≤ nfp F 0 → nfp F a = nfp F 0`,
describes the same route — ⚠ **though not identically: § I-b uses two `nfp_le_fp` applications with
`nfp_fp`, where the proof below uses one plus `epsilon0_least_fixedpoint`** — fences **normality** as
load-bearing (*"the displayed implication is false
without it"*), and draws the same conclusion — *"⊥ is **a** seed, not a distinguished one"* — under its
own verdict, ***"Elementary and not novel."*** **What the declarations below add is only that the prose
becomes checkable.**

**Prior art — the statement below is CLASSICAL, seed-range and all.** Veblen, *Continuous increasing
functions of finite and transfinite ordinals*, Trans. Amer. Math. Soc. 9 (1908), read from source and
...
* **Corollary 4**: *"The first derived function of ωˣ is the function ε"* — what licenses reading (A)
  at `f = ωˣ`.
...
**Residual delta against Veblen: the seed `0`, and the strict interior `1 < a < ε₀`.** Veblen's
indexing starts at `1`, so `a = 0` is outside his range — Mathlib's `epsilon_zero_eq_nfp` is its home —
and the interior is routine monotone filling between (A) and (B).
...
theorems G and H, p. 245."* ⚠ **Reading that PDF: in those two footnotes the OCR renders `§` as a backslash and the dagger
...
classical anchor is Corollary 1 clause (A) plus monotonicity, and the declaration below is that in Lean.
...
ascent μ (seed ⊥ ≠ closure ε₀)"*. **That ascent is genuine.** What the declarations add is that **within
`[0, ε₀]` the seed carries no information**: seeding at `1`, at `ω`, at `ω^ω` gives the same ε₀.
...
* **No ε-number can test it.** Every ε-number is a fixed point, so `nfp (ω^·) ε₁ = ε₁` and likewise
  at every rung — the whole family is blind to the difference between the general fact and
  *"above ε₀ the seed does all the work"*, which is false. Test above ε₀ with a **non**-fixed point
  such as `succ ε₀`.
* **The rungs do NOT partition.** The intervals `(ε_o, ε_(o+1)]` miss every **limit**-index ε-number,
  e.g. `ε_ω`.
* **Normality is essential**, per § I-b's fence: the implication fails for a non-normal operator.
