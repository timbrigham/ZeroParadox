"""
Build ZP-A: Lattice Algebra (v1.23)
...
def build():
    ...
    E.append(label_box('OQ-A1 — Sufficiency of Monotonicity  [CLOSED within ZP given AX-B1 and the occurrence commitment — ZP-E T5]', [
        ...
        'Status: CLOSED within ZP given AX-B1 and the occurrence commitment — ZP-E T5 (Iterative Forcing Theorem), restated in ZP-E: its selection half is a Conditional Claim given AX-B1&#8217;s discreteness at every state, and that each step is taken is the occurrence commitment applied at each step. OQ-A1a: if the step from S<sub>n</sub> is taken, &#945;<sub>n</sub> = &#949;(S<sub>n</sub>), the minimum viable deviation (ZP-specific). OQ-A1b: yes. AX-B1 does not bound ascending chains: its discreteness gives a minimum step above each state, not a limit on how many steps are taken. The natural numbers show both at once: every n has the immediate successor n + 1 above it, and 0 &lt; 1 &lt; 2 &lt; &#8230; ascends with no upper bound. In a semilattice without top a strictly greater state is always available; that a chain takes every step is occurrence, not a property of the carrier. (A carrier with a top, such as the two-state MachinePhase, bounds its chains by having that top, not by AX-B1.) In general semilattice theory without AX-B1 unbounded ascending chains are likewise permitted — a bare join-semilattice imposes no bound. The closure here is a consequence of ZP&#8217;s binary existence commitment and the occurrence commitment, not of A1&#8211;A4 alone.',
    ...
    E.append(data_table(
        ...
        [['(L, &#8744;, &#8869;) as join-semilattice', 'Derived (A1&#8211;A4) — ZP-D: algebraic structure of state space'],
         ...
         ['OQ-A1 — increment selection', 'Open within ZP-A; closed within ZP given AX-B1 and the occurrence commitment (ZP-E T5)']],
        [2.5*inch, 4.0*inch]
    ))
    ...
    E.append(data_table(
        ...
        [['A1&#8211;A4 join-semilattice axioms', 'Valid — Axioms; self-contained'],
         ...
         ['OQ-A1: Sufficiency of monotonicity', 'Open within ZP-A; closed within ZP given AX-B1 and the occurrence commitment (ZP-E T5)']],
        [2.5*inch, 4.0*inch]
    ))
