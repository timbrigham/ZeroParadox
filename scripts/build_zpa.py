"""
Build ZP-A: Lattice Algebra (v1.24)
...
def build():
    ...
    E.append(label_box('OQ-A1 — Sufficiency of Monotonicity  [OQ-A1b CLOSED by A1&#8211;A4; OQ-A1a answered by crossing charts (snap_exactly_at_epsilon_zero)]', [
        ...
        'OQ-A1a, answered by crossing charts: map the ordinal tower into the lattice by a monotone &#966; sending the tower&#8217;s stages to c<sub>0</sub> and &#949;<sub>0</sub> to c<sub>1</sub>; the lattice step c<sub>0</sub> &#8594; c<sub>1</sub> then happens at &#949;<sub>0</sub> and at no ordinal below it (snap_exactly_at_epsilon_zero). The step from any seed is the least fixed point at or above it (isLeastFixedPointFrom_nfp), and the next rung re-seeds one step past the last (succession_succ). That the step occurs is a hypothesis there (h&#949;<sub>0</sub>). On an arbitrary lattice nothing restricts which increment is taken: 0, 2, 4, &#8230; on the natural numbers satisfies A1&#8211;A4 and AX-B1.',
        ...
        'Note on minimal element above &#8869;: Bounded chains do not in themselves guarantee a minimal element above &#8869;. In a dense structure, for any &#949; > 0, &#949;/2 also exists &#8212; chains may be bounded yet have no first step. That is not a hypothetical: it is ZP-F, where the halving argument is proved for every field carrying a compatible linear order (f_snap_impossible, ZeroParadox/Reals/OrderedField.lean), and is stated there as the direct negation of AX-B1 (axb1_fails_in_ordered_field). The closure of OQ-A1b does not bound ascending chains, and a bound would not establish that &#949;&#8320; exists as a specific minimal non-null element either. That existence rests on the same binary existence commitment (AX-B1), not on a further result. ZP-B&#8217;s 2-adic structure places 0 at infinite valuation and every non-zero element at finite valuation; what it proves there is that the gap at 0 is clopen (T3) and the return across it irreversible (C3). The non-zero elements still come arbitrarily close to 0 (&#8214;2&#8319;&#8214;&#8322; = 2&#8315;&#8319;), so there is no closest non-zero element to serve as a first step. This is outside the scope of A1&#8211;A4.',
    ]))
