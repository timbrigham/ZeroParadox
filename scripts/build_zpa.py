"""
Build ZP-A: Lattice Algebra (v1.25)
...
def build():
    ...
    E.append(label_box('OQ-A1 — Sufficiency of Monotonicity  [OQ-A1b CLOSED by A1&#8211;A4; OQ-A1a answered by crossing charts (snap_exactly_at_epsilon_zero)]', [
        ...
        'OQ-A1a, answered by crossing charts: map the ordinal tower into the lattice by a monotone &#966; sending the tower&#8217;s stages to c<sub>0</sub> and &#949;<sub>0</sub> to c<sub>1</sub>; the lattice step c<sub>0</sub> &#8594; c<sub>1</sub> then happens at &#949;<sub>0</sub> and at no ordinal below it (snap_unconditional). The step from any seed is the least fixed point at or above it (isLeastFixedPointFrom_nfp), and the next rung re-seeds one step past the last (succession_succ). That the step occurs is a hypothesis there (h&#949;<sub>0</sub>). On an arbitrary lattice nothing restricts which increment is taken: 0, 2, 4, &#8230; on the natural numbers satisfies A1&#8211;A4 and AX-B1.',
        'No step is guaranteed. A state sequence moves only upward (T3) and never returns (R1, t_snap_irreversible). Where the next increment is already absorbed (S<sub>n</sub> &#8852; &#945;<sub>n</sub> = S<sub>n</sub>), the state stays the same, which T-SNAP permits (tsnap_holds_but_nothing_moves); different runs need not reach the same height. If from some step x on every new increment is already absorbed, then S<sub>x</sub> is the supremum of the run: the value that run reaches in the limit. Reaching it cannot be confirmed at any finite step.',
        'Reading: &#949;<sub>0</sub> is this pair for the ordinal tower, the least fixed point of &#945; &#8614; &#969;<super>&#945;</super> and the supremum of its stages (epsilon0_min_eq_max).',
        'Note on minimal element above &#8869;: Bounded chains do not in themselves guarantee a minimal element above &#8869;. In a dense structure, for any &#949; > 0, &#949;/2 also exists &#8212; chains may be bounded yet have no first step. That is not a hypothetical: it is ZP-F, where the halving argument is proved for every field carrying a compatible linear order (f_snap_impossible, ZeroParadox/Reals/OrderedField.lean), and is stated there as the direct negation of AX-B1 (axb1_fails_in_ordered_field). The closure of OQ-A1b does not bound ascending chains, and a bound would not establish that &#949;&#8320; exists as a specific minimal non-null element either. In the lattice chart that existence rests on the same binary existence commitment (AX-B1); in the ordinal chart &#949;<sub>0</sub> is proved to be the least fixed point of &#945; &#8614; &#969;<super>&#945;</super> (epsilon0_min_eq_max), and, given that the step occurs, a monotone map places the lattice step there (snap_unconditional). ZP-B&#8217;s 2-adic structure places 0 at infinite valuation and every non-zero element at finite valuation; what it proves there is that the gap at 0 is clopen (T3) and the return across it irreversible (C3). The non-zero elements still come arbitrarily close to 0 (&#8214;2&#8319;&#8214;&#8322; = 2&#8315;&#8319;), so there is no closest non-zero element to serve as a first step. This is outside the scope of A1&#8211;A4.',
    ]))
    ...
    E.append(data_table(
        ...
        [['(L, &#8744;, &#8869;) as join-semilattice', 'Derived (A1&#8211;A4) — ZP-D: algebraic structure of state space'],
         ...
         ['OQ-A1 — increment selection', 'OQ-A1b CLOSED by A1&#8211;A4; OQ-A1a answered by crossing charts (snap_exactly_at_epsilon_zero)']],
        [2.5*inch, 4.0*inch]
    ))
    ...
    E.append(data_table(
        ...
        [['A1&#8211;A4 join-semilattice axioms', 'Valid — Axioms; self-contained'],
         ...
         ['OQ-A1: Sufficiency of monotonicity', 'OQ-A1b CLOSED by A1&#8211;A4; OQ-A1a answered by crossing charts (snap_exactly_at_epsilon_zero)']],
        [2.5*inch, 4.0*inch]
    ))
