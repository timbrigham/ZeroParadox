...
v1.5: DA-1/KLEENE CLASS, GATE ROUND 4 SECOND PASS (Tim ruling, 2026-09-15): the verification box said Classical.choice is load-bearing in the computability fixed point; the choice there belongs to ZP-K's instance (its choice of botCode), and a computable instance with a constant code also exists. The ordinal half of the sentence is unchanged.
...
def build():
    ...
    E.append(axiom_box(
        ...
        [
            ...
            'the computability fixed-point (Kleene\'s theorem). In the computability layer '
            'Classical.choice is carried by the statements\' types, through Mathlib\'s numbering '
            'of program codes (Denumerable Code): it is present even in a computable instance '
            'with a constant code. ZP-K\'s machinePhaseKleene also picks botCode with '
            'Classical.choose, which is what makes that instance noncomputable. Whether the '
            'numbering\'s footprint is essential is not measured. Its presence is expected and '
            'documented.',
