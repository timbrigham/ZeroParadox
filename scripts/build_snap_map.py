...
IS_FACES = [
    ("theorem", "the *derived* (not assumed) transition off ⊥ into the minimum non-⊥ state of the discrete state chart - derived, which is a claim about its provenance and not that it is taken: the join c₀ ∨ c₁ = c₁ is a valid transition, and c₀, c₁ are provably distinct in both directions. AX-1 (the Binary Snap) is retired: its shape is proved, as Theorem T-SNAP, and that the snap occurs is stated separately: it follows from the occurrence commitment (instantiation occurs) together with DA-1 (closed given DP-2)",
     ["t_snap_derived"]),
...
IS_NOT = [
    ...
    ("*numerically one transition across its carriers*. The valuation, categorical, order, and computability snaps form one **family** (MC-1), not one object: each is a member (membership proved per domain), but the reading that they are numerically the same object is **retired** as ill-typed (neither x = y nor x ≠ y is well-formed across categories); the members differ by property, each in its own category (seam_unique_among_named). What they share is the diagonal-fixed-point *shape*, not an identity",
     []),
    ("*a physical, temporal, or causal event*. The framework is silent on physics. The snap is an order and derivation transition, not a process unfolding in time; which specific state emerges first is outside its scope",
     []),
    ("*dependent on a snap-specific axiom*. T-SNAP is derived from the bottom axiom A4 (the join identity ∀ x, ⊥ ∨ x = x) and the framework's commitments; `t_snap_given` states two of them, the start at ⊥ and occurrence at the first step, as its hypotheses. No snap axiom appears anywhere in the development: AX-1 is retired, and that the snap occurs follows from the occurrence commitment together with DA-1, neither of them an axiom",
...
BOUNDARY_MAP = [
    ...
    ("order / set theory", "**Available** (choice-free spine)",
     "the fork collapses to the diagonal fixed point exactly when the map has a unique fixed point, and the self-containing ⊥ = {⊥} realizes it. The field snaps form one family (MC-1); the reading that they are numerically one object is retired as ill-typed (neither x = y nor x ≠ y is well-formed across categories); the members differ by property, each in its own category (`seam_unique_among_named`)",
...
**Available does not mean it happens.** No field in this table establishes that the snap *occurs* - that follows from the occurrence commitment (instantiation occurs) together with DA-1 (closed given DP-2), and tsnap_holds_but_nothing_moves exhibits a model in which T-SNAP holds and nothing moves. What each row records is that the field's own structure supplies the ingredients and raises no obstruction. One row is different in kind: in the reals the snap is provably blocked.

The pattern is worth stating plainly. The self-referential *shape* - the diagonal fixed point - recurs across every face, but the faces are not one object across them - they are one **family** (MC-1): the numerical identity is retired as ill-typed (neither x = y nor x ≠ y is well-formed across categories); the members differ by property, each in its own category ([`seam_unique_among_named`](ZeroParadox/Category/SeamUniqueness.lean)). What is available across almost every field is the *snap itself*, each by its own native mechanism. One field is the telling exception: in the real numbers the snap provably fails, and the failure is a theorem. Every row here cites machine-checked results; what singles out the reals is that theirs records an *obstruction* where the others record a *mechanism*.
...
**Proved - the snap is DERIVED rather than assumed, and adds no axiom.** T-SNAP (t_snap_derived): the transition ⊥ → ε₀ (in the discrete state chart, ε₀ names the minimum non-⊥ state; the ordinal ε₀ is a different chart, at once the least fixed point of α ↦ ω^α and the supremum of its tower, epsilon0_min_eq_max, indexed in `ZeroParadox/Ordinal/Epsilon0CannotBe.lean`) is a derived consequence of the bottom axiom A4 and the framework's computational commitments, not an assumption. **"Derived" is a claim about where the transition comes from, not that it is taken:** T-SNAP constrains the SHAPE of the transition and the same file's tsnap_holds_but_nothing_moves proves it holds where nothing moves. That the snap occurs is not a theorem. Earlier layers posited the Binary Snap as AX-1. AX-1 is retired: its content was split in two, the shape proved as Theorem T-SNAP, and that the snap occurs stated separately, following from the occurrence commitment (instantiation occurs) together with DA-1 (closed given DP-2); no snap-specific axiom appears anywhere.
...
**The family (proved), the identity (retired), choice-free minimality (open).** The domain snaps form one **family** (MC-1): per-domain membership is proved, the reading that they are *numerically one* transition is retired as ill-typed (neither x = y nor x ≠ y is well-formed across categories); the members differ by property, each in its own category ([`seam_unique_among_named`](ZeroParadox/Category/SeamUniqueness.lean)). Separately, ε₀-as-least-fixed-point is proved classically; whether it can be shown choice-free at the notation level is open: the syntax-to-semantics bridge tower_NF inherits `Classical.choice`.
...
It is a **beginning, not a resolution.** What is *proved* is that the snap's shape is derived rather than assumed (that it occurs follows from the occurrence commitment together with DA-1, closed given DP-2), one-way, takes its first step from below, and that the domain snaps form one **family** (MC-1, membership proved per domain); what is *retired* is the reading that they are numerically one object (ill-typed: neither x = y nor x ≠ y is well-formed across categories; the members differ by property, each in its own category, seam_unique_among_named); and what stays *open* is the choice-freeness of the first step's minimality. The frame-change faces are proved theorems that compose known results, so no new theorem is claimed there; what stays a conjecture is only the abstract cross-domain reading - that they are literally one change of frame (a type boundary) - written up in [ZP-Q](ZP-Q_The_Frame_Change.pdf).
