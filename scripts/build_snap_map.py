...
IS_NOT = [
    ...
    ("*numerically one transition across its carriers*. The valuation, categorical, order, and computability snaps form one **family** (MC-1), not one object: each is a member (membership proved per domain), but the reading that they are numerically the same object is **retired** as ill-typed (object equality across categories does not typecheck and is not invariant under equivalence). What they share is the diagonal-fixed-point *shape*, not an identity",
     []),
...
BOUNDARY_MAP = [
    ...
    ("order / set theory", "**Available** (choice-free spine)",
     "the fork collapses to the diagonal fixed point exactly when the map has a unique fixed point, and the self-containing ⊥ = {⊥} realizes it. The field snaps form one family (MC-1); the reading that they are numerically one object is retired as ill-typed (object equality across categories does not typecheck and is not invariant under equivalence)",
...
**The family (proved), the identity (retired), choice-free minimality (open).** The domain snaps form one **family** (MC-1): per-domain membership is proved, the reading that they are *numerically one* transition is retired as ill-typed (object equality across categories does not typecheck and is not invariant under equivalence). Separately, ε₀-as-least-fixed-point is proved classically; whether it can be shown choice-free at the notation level is open: the syntax-to-semantics bridge tower_NF inherits `Classical.choice`.
