...
**Proved - each field's own floor.** 0 in the 2-adics, where v₂(0) = ∞ (addVal_bot); unbounded surprisal, the state with no finite description (t2_diverges); the categorical bottom of each real Mathlib category, an inverse limit or initial object (mc1_correspondence); and the case where the correspondence *fails*, ℝ vs ℚ₂ by Ostrowski (completions_exhaustive, real_not_equiv_padic). They all instantiate one abstract schema, choice-free (fork_collapse_iff); and ε₀ is co-witnessed with the 2-adic limit and the machine snap (zpm_triangle).
