...
def scan():
    ...
    for dp, _, files in os.walk(SRC):
        for fn in sorted(files):
            ...
            title = None
            for i, ln in enumerate(lines):         # 1st choice: a markdown `# Title` (the common `/-! # ...` style)
                m = TITLE_RE.match(ln)
                if m:
                    title = m.group(1)
                    # A header that WRAPS to a second line would otherwise yield a fragment row.
                    # Join continuation lines: non-blank, not a new heading, not the end of the
                    # docstring, and not the start of the body (a `##` section or a `-`/`*` bullet).
                    # Measured 2026-08-02: two MANIFEST rows read as sentence fragments because this
                    # loop took only the first physical line of a wrapped title.
...
def main():
    ...
    # newline="" keeps LF: without it Python translates \n to \r\n on Windows, and every
    # regeneration then trips git's CRLF warning against the repo's `* text=auto eol=lf`.
